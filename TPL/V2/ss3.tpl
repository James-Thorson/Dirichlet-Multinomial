      ...
      if(gen_l(f,i)!=2)
      {
        ...
        for (z=tails_l(f,i,1);z<=tails_l(f,i,2);z++)
        {
            ...
            if(nsamp_l(f,i)>0 && header_l(f,i,3)>0)
            {
              ...
              // BEGIN NEW LINES
              if(Comp_Err_L(f)==0) SS_compout<<value((obs_a(f,i,z)-exp_a(f,i,z))/sqrt( exp_a(f,i,z) * (1.0-exp_a(f,i,z)) / sfabs(nsamp_l(f,i)))); // Pearson for multinomial
              if(Comp_Err_L(f)==1){
        ...
        if(gen_a(f,i)!=2)
         ...
         <<" "<<s_off<<" "<<len_bins(Lbin_lo(f,i))<<" "<<len_bins(Lbin_hi(f,i))<<" "<<age_bins(z)<<" "<<obs_a(f,i,z)<<" " <<exp_a(f,i,z)<<" ";
          ...
          if(header_a(f,i,3)>0)
          {
            ...
            {
              // BEGIN NEW LINES
              if(Comp_Err_L(f)==0) SS_compout<<value((obs_a(f,i,z)-exp_a(f,i,z))/sqrt( exp_a(f,i,z) * (1.0-exp_a(f,i,z)) / sfabs(nsamp_a(f,i)))); // Pearson for multinomial
              if(Comp_Err_L(f)==1){
