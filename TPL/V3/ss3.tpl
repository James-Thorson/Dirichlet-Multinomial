...
// from Thorson:  NLL -= gammln(A) - gammln(ninput_t(t)+A) + sum(gammln(ninput_t(t)*extract_row(pobs_ta,t) + A*extract_row(pexp_ta,t))) - sum(lgamma(A*extract_row(pexp_ta,t))) \
        // BEGIN NEW LINES
        if(Comp_Err_L(f)==1) dirichlet_Parm=mfexp(selparm(Comp_Err_L2(f)))*nsamp_l(f,i);
        if(Comp_Err_L(f)==2) dirichlet_Parm=mfexp(selparm(Comp_Err_L2(f)));
        // END NEW LINES
...
// from Thorson:  NLL -= gammln(A) - gammln(ninput_t(t)+A) + sum(gammln(ninput_t(t)*extract_row(pobs_ta,t) + A*extract_row(pexp_ta,t))) - sum(lgamma(A*extract_row(pexp_ta,t))) \
              //cout<<"f "<<f<<" i "<<i<<endl;
            // BEGIN NEW LINES
              if(Comp_Err_A(f)==1) dirichlet_Parm=mfexp(selparm(Comp_Err_A2(f)))*nsamp_a(f,i);
              if(Comp_Err_A(f)==2) dirichlet_Parm=mfexp(selparm(Comp_Err_A2(f)));
            // END NEW LINES
              ...
              //cout<<"selparm "<<selparm<<endl;              
              //cout<<"selparm(Comp_Err_A2) "<<selparm(Comp_Err_A2(f))<<endl;
              //cout<<"nsamp_a(f,i) "<<nsamp_a(f,i)<<endl;
              //cout<<"dirichlet_Parm "<<dirichlet_Parm<<endl;
              //cout<<"Comp_Err_A "<<Comp_Err_A<<endl;
              //cout<<"Comp_Err_A2 "<<Comp_Err_A2<<endl;
              //cout<<"temp "<<temp<<endl;
              age_like(f,i)-=temp;
            }
          }
...
//   cout<<" obj_fun parm "<<obj_fun<<endl;
   obj_fun += (sum(MGparm_dev_like)+sum(selparm_dev_like))*parm_dev_lambda(k_phase);
...
//  need to replace this with Dirichlet equivalent
           k=1000;
           if(Comp_Err_L(f)==0 && (nsamp_l(f,i)<k)) k=nsamp_l(f,i);
           if(Comp_Err_L(f)==1) k=int(mfexp(value(selparm(Comp_Err_L2(f))))*nsamp_l(f,i));  // Rick
           if(Comp_Err_L(f)==2) k=int(mfexp(value(selparm(Comp_Err_L2(f)))));  //  Rick
           if(k==0) k=1;
   ...
   for (f=1;f<=Nfleet;f++)
    ...
    if(Nobs_a(f)>=1)
    {
     ...
     if(header_a(f,i,3)>0) // if real observation
     {
      ...
      {
        //  need to replace this with code for dirichlet  //  RICK
        k=1000;
        if(Comp_Err_A(f)==0 && (nsamp_a(f,i)<k)) k=nsamp_a(f,i);
        if(Comp_Err_A(f)==1) k=int(mfexp(value(selparm(Comp_Err_A2(f))))*nsamp_a(f,i));  // Rick
        if(Comp_Err_A(f)==2) k=int(mfexp(value(selparm(Comp_Err_A2(f)))));  //  Rick
        if(k==0) k=1;
// note that nsamp is adjusted by var_adjust, so var_adjust
// should be reset to 1.0 in control files that read the nudata.dat files
        exp_a_temp = 0.0;
        temp_probs2 = value(exp_a(f,i));
        temp_mult.fill_multinomial(radm,temp_probs2);
      ...
      if(gen_l(f,i)!=2)
      {
        ...
        for (z=tails_l(f,i,1);z<=tails_l(f,i,2);z++)
        {
            ...
            if(nsamp_l(f,i)>0 && header_l(f,i,3)>0)
            {
              ...
              // BEGIN NEW LINES
              if(Comp_Err_L(f)==0) SS_compout<<value((obs_l(f,i,z)-exp_l(f,i,z))/sqrt( exp_l(f,i,z) * (1.0-exp_l(f,i,z)) / sfabs(nsamp_l(f,i)))); // Pearson for multinomial  //RICK
              if(Comp_Err_L(f)==1){
                dirichlet_Parm=mfexp(selparm(Comp_Err_L2(f)))*nsamp_l(f,i);
                SS_compout<<value( (obs_l(f,i,z)-exp_l(f,i,z))/sqrt( exp_l(f,i,z) * (1.0-exp_l(f,i,z)) / sfabs(nsamp_l(f,i)) * (sfabs(nsamp_l(f,i))+dirichlet_Parm)/(1+dirichlet_Parm) ) );
              }
              if(Comp_Err_L(f)==2){
                dirichlet_Parm=mfexp(selparm(Comp_Err_L2(f)));
                SS_compout<<value((obs_l(f,i,z)-exp_l(f,i,z))/sqrt( exp_l(f,i,z) * (1.0-exp_l(f,i,z)) / sfabs(nsamp_l(f,i))));
              }
              // END NEW LINES
              }
              else
        ...
        if(gen_a(f,i)!=2)
         ...
         <<" "<<s_off<<" "<<len_bins(Lbin_lo(f,i))<<" "<<len_bins(Lbin_hi(f,i))<<" "<<age_bins(z)<<" "<<obs_a(f,i,z)<<" " <<exp_a(f,i,z)<<" ";
          ...
          if(header_a(f,i,3)>0)
          {
            ...
            {
              // BEGIN NEW LINES
              if(Comp_Err_A(f)==0) SS_compout<<value((obs_a(f,i,z)-exp_a(f,i,z))/sqrt( exp_a(f,i,z) * (1.0-exp_a(f,i,z)) / sfabs(nsamp_a(f,i)))); // Pearson for multinomial  // RICK
              if(Comp_Err_A(f)==1){     // RICK
                dirichlet_Parm=mfexp(selparm(Comp_Err_A2(f)))*nsamp_a(f,i);
                SS_compout<<value( (obs_a(f,i,z)-exp_a(f,i,z))/sqrt( exp_a(f,i,z) * (1.0-exp_a(f,i,z)) / sfabs(nsamp_a(f,i)) * (sfabs(nsamp_a(f,i))+dirichlet_Parm)/(1+dirichlet_Parm) ) );
              }
              if(Comp_Err_A(f)==2){  //  RICK
                dirichlet_Parm=mfexp(selparm(Comp_Err_A2(f)));
