...
if(!exists('dm4')) dm4 <- SS_output(file.path(dir, 'hake_V3.3_explicitF_V4'))
source(file.path(dir,'R/SSplotComps_Dirichlet-Multinomial.R'))

## Pearson residuals
doPNG <- T
ht <- 7; wd<- 6
if(doPNG) {png(file.path(dir,"results/ageCompPearsons.png"),height=ht,width=wd,
