      ...
      for(j in unique(dbase_k$Part)){
        ...
        if(2 %in% subplots & bub & kind!="cond"){
          ...
          tempfun2 <- function(){
            ...
            r4ss:::bubble3(x=xvals, y=dbase$Bin, z=z, xlab=labels[3],
          ...
          for(y in 1:length(aalyear)){
            ...
            if(length(dbase$Obs[dbase$Yr==aalyr])>0){
              ...
              if(5 %in% subplots){
                ...
                titles <- c(ptitle,titles) # compiling list of all plot titles
                tempfun5 <- function(){
                  r4ss:::bubble3(x=ydbase$Bin,y=ydbase$Lbin_lo,z=z,xlab=labels[2],
...
# for Dirichlet-Multinomial paper, hardwire the axis labels
xaxislab <- seq(1975,2015,5)
          # get growth curves if requested
          if(length(cohortlines)>0){
            growdat <- replist$endgrowth
            growdatF <- growdat[growdat$Gender==1 & growdat$Morph==min(growdat$Morph[growdat$Gender==1]),]
          ...
          multifleet.bubble.fun <- function(ipage=0){
            ...
            par(mfrow=c(min(npanels,maxrows),1),
                mar=c(0.5,0,0,0),
# for Dirichlet-Multinomial paper, hardwire the axis ticks
                oma=c(4,5,1,1))
            ...
            for(f in fleetvec[panelrange]){
              ...
              r4ss:::bubble3(x=dbase$Yr.S, y=dbase$Bin, z=z, col=cols, cexZ1=cexZ1,
                      legend=bublegend,
...
# for Dirichlet-Multinomial paper, make labels closer
              mtext(fleetnames[f],side=2,line=3,cex=par()$cex)
              ...
              if(par()$mfg[1]==par()$mfg[3] | f==tail(fleetvec,1)){
                ...
                axis(1,at=xaxislab)
# for Dirichlet-Multinomial paper, hardwire the axis ticks
axis(1,at=1900:2100, labels=rep("",201), tcl = -0.2)                
              }else{
                # or just tick marks for other panels
                axis(1,at=xaxislab,labels=rep("",length(xaxislab)))
axis(1,at=1900:2100, labels=rep("",201))                
axis(1,at=1900:2100, labels=rep("",201))                
              }
# for Dirichlet-Multinomial paper, comment comment out the title
#              if(par()$mfg[1]==1)
